...
// 构造函数，初始化 GeoData 对象
GeoData::GeoData()
    : SketchObject(ESketchObjectType::GeoData), // 初始化基类 SketchObject，设置对象类型为 GeoData
      IRenderable(), // 初始化基类 IRenderable
      m_dirty_flags(0xffffffff), // 初始化脏标志位为 0xffffffff
      m_nous_mesh(nous::NMesh::create()), // 创建并初始化 m_nous_mesh 对象
      m_nous_position_handle(m_nous_mesh->point_attrib<nous::pos_t>(nous::ATTRIB_NAME_POS)) // 初始化 m_nous_position_handle，指向 m_nous_mesh 的位置属性
{
}

// 拷贝构造函数，初始化 GeoData 对象为另一个 GeoData 对象的副本
GeoData::GeoData(const GeoData& other)
    : SketchObject(ESketchObjectType::GeoData), // 初始化基类 SketchObject，设置对象类型为 GeoData
      m_nous_mesh(nous::NMesh::create()), // 创建并初始化 m_nous_mesh 对象
      m_nous_position_handle(m_nous_mesh->point_attrib<nous::pos_t>(nous::ATTRIB_NAME_POS)) // 初始化 m_nous_position_handle，指向 m_nous_mesh 的位置属性
{
    // 复制 other 对象的 m_nous_mesh 到当前对象的 m_nous_mesh
    m_nous_mesh->copy(other.m_nous_mesh.get());

    // 复制点
    int num_points = m_nous_mesh->num_points(); // 获取 m_nous_mesh 中点的数量
    for(int i = 0; i < num_points; ++i) // 遍历每个点
    {
        nous::Point* point = m_nous_mesh->point(i); // 获取当前点的指针
        m_nousptr_points[point] = std::make_shared<SketchPoint>(point); // 创建 SketchPoint 对象并存储在 m_nousptr_points 中
    }

    // 复制线
    for(const auto& pair: other.m_key_lines) // 遍历 other 对象的 m_key_lines 容器
    {
        nous::Prim* line_prim = m_nous_mesh->prim(pair.second->line_prim->idx()); // 获取当前线的 prim 对象
        size_t point0_idx = pair.second->p0->point->idx(); // 获取线的第一个点的索引
        size_t point1_idx = pair.second->p1->point->idx(); // 获取线的第二个点的索引
        nous::Point* point0 = m_nous_mesh->point(point0_idx); // 获取第一个点的指针
        nous::Point* point1 = m_nous_mesh->point(point1_idx); // 获取第二个点的指针

        size_t lk = line_key(m_nousptr_points.at(point0), m_nousptr_points.at(point1)); // 计算线的键
        m_key_lines[lk] = std::make_shared<SketchLine>(line_prim, lk); // 创建 SketchLine 对象并存储在 m_key_lines 中
        m_key_lines[lk]->p0 = m_nousptr_points.at(point0); // 设置线的第一个点
        m_key_lines[lk]->p1 = m_nousptr_points.at(point1); // 设置线的第二个点
        m_nousptr_lines[line_prim] = m_key_lines[lk]; // 将线对象存储在 m_nousptr_lines 中
    }
    // 复制环
    for(const auto& pair: other.m_key_rings) // 遍历 other 对象的 m_key_rings 容器
    {
        nous::Prim* ring_prim = m_nous_mesh->prim(pair.second->ring_prim->idx()); // 获取当前环的 prim 对象

        // 获取环的点
        std::vector<SketchPointPtr> ring_points; // 定义一个向量存储环的点
        for(auto vtx_idx: m_nous_mesh->prim_vertices_range(ring_prim->idx())) // 遍历环的顶点索引
        {
            ring_points.push_back(m_nousptr_points.at(m_nous_mesh->vertex(vtx_idx)->point_ptr())); // 获取顶点对应的点并添加到 ring_points 中
        }

        size_t ring_key = GeoData::ring_key(ring_points); // 计算环的键
        SketchRingPtr ring = std::make_shared<SketchRing>(ring_prim, ring_key); // 创建 SketchRing 对象
        ring->normal = pair.second->normal; // 设置环的法线
        m_key_rings[ring_key] = ring; // 将环对象存储在 m_key_rings 中
        m_nousptr_rings[ring_prim] = ring; // 将环对象存储在 m_nousptr_rings 中
    }
    // 复制多边形
    for(auto& ring_relation: other.m_ring_adj_polygons) // 遍历 other 对象的 m_ring_adj_polygons 容器
    {
        nous::Prim* ring_prim = m_nous_mesh->prim(ring_relation.first->ring_prim->idx()); // 获取当前环的 prim 对象
        SketchRingPtr ring = m_nousptr_rings.at(ring_prim); // 获取对应的 SketchRing 对象

        if(ring_relation.second.polygon) // 检查是否存在多边形
        {
            auto polygon_pos = m_ring_adj_polygons.find(ring); // 查找当前环是否已经存在于 m_ring_adj_polygons 中
            if(polygon_pos == m_ring_adj_polygons.end()) // 如果不存在
            {
                SketchPolygonPtr current_polygon = std::make_shared<SketchPolygon>(ring); // 创建新的 SketchPolygon 对象
                current_polygon->normal = ring_relation.second.polygon->normal; // 设置多边形的法线
                m_ring_adj_polygons[ring].polygon = current_polygon; // 将多边形对象存储在 m_ring_adj_polygons 中
            }
            else if(!polygon_pos->second.polygon) // 如果存在但没有多边形对象
            {
                SketchPolygonPtr current_polygon = std::make_shared<SketchPolygon>(ring); // 创建新的 SketchPolygon 对象
                current_polygon->normal = ring_relation.second.polygon->normal; // 设置多边形的法线
                polygon_pos->second.polygon = current_polygon; // 将多边形对象存储在 m_ring_adj_polygons 中
            }
            for(auto& inner_ring: ring_relation.second.polygon->inner_rings) // 遍历多边形的内环
            {
                nous::Prim* inner_ring_prim = m_nous_mesh->prim(inner_ring->ring_prim->idx()); // 获取内环的 prim 对象
                SketchRingPtr inner_ring = m_nousptr_rings.at(inner_ring_prim); // 获取对应的 SketchRing 对象
                m_ring_adj_polygons[ring].polygon->inner_rings.insert(inner_ring); // 将内环添加到多边形的内环集合中
                m_ring_adj_polygons[inner_ring].parent_polygon = m_ring_adj_polygons[ring].polygon; // 设置内环的父多边形
            }
        }
        if(ring_relation.second.parent_polygon) // 检查是否存在父多边形
        {
            nous::Prim* parent_ring_prim = m_nous_mesh->prim(ring_relation.second.parent_polygon->ring->ring_prim->idx()); // 获取父环的 prim 对象
            SketchRingPtr parent_ring = m_nousptr_rings.at(parent_ring_prim); // 获取对应的 SketchRing 对象
            // 处理父多边形
            auto parent_pos = m_ring_adj_polygons.find(parent_ring); // 查找父环是否已经存在于 m_ring_adj_polygons 中
            if(parent_pos == m_ring_adj_polygons.end()) // 如果不存在
            {
                SketchPolygonPtr parent_skp = std::make_shared<SketchPolygon>(parent_ring); // 创建新的 SketchPolygon 对象
                parent_skp->normal = ring_relation.second.parent_polygon->normal; // 设置父多边形的法线
                m_ring_adj_polygons[parent_ring].polygon = parent_skp; // 将父多边形对象存储在 m_ring_adj_polygons 中
            }
            m_ring_adj_polygons[ring].parent_polygon = m_ring_adj_polygons[parent_ring].polygon; // 设置当前环的父多边形
        }
    }
    // group
...
std::vector<SketchLinePtr> GeoData::add_line(const nous::vec3& p0, const nous::vec3& p1, bool genPolygon)
{
    ...
    // 查找受影响的焊接组
    std::unordered_set<ElementGroupPtr> weld_line_groups;
    for(auto line: recreat_line)
    {
        if(line.first->group() && line.first->group()->type() == EElementType::NoBranchLineGroup)
        {
            weld_line_groups.insert(line.first->group());
        }
    }
    ...
    for(auto point: new_line_points)
    {
        ...
        {
            if(adj_line->group() && adj_line->group()->type() == EElementType::NoBranchLineGroup)
            {
                weld_line_groups.insert(adj_line->group());
            }
        }
    }
    ...
    // 标记被影响的焊接组
    // 需要重新焊接原因：部分与端点相交的焊接组，虽然拓扑不变，但依然需要被分割。
    for(auto group: weld_line_groups)
    {
...
std::vector<std::vector<SketchPointPtr>> GeoData::add_polygon(const std::vector<std::vector<nous::pos_t>>& polygon_pos)
{
    ...
    for(const auto& ring: polygon_pos)
    {
        ...
        for(int i = 0; i < ring.size(); ++i)
        {
            ...
            std::vector<SketchPointPtr> new_lines;
            std::map<SketchLinePtr, std::vector<SketchPointPtr>> recreat_line;
            _add_line(p0, p1, recreat_line, new_lines);
            ...
            if(new_lines.size())
            {
                ...
                {
                    recreat_line_points.insert(new_lines[i]);
                    ring_pt_arr.push_back(new_lines[i]);
                    auto& cl = _add_nous_line(new_lines[i], new_lines[i + 1]);
                }
                recreat_line_points.insert(new_lines.back());
            }
...
void GeoData::add_polygon(const std::vector<std::vector<SketchPointPtr>>& polygon, nous::vec3 normal)
{
    ...
    for(const auto& polygon: get_all_polygons())
    {
        const nous::vec3 ring_normal = polygon->normal;
        const nous::pos_t ring_pos = get_point_position(m_nousptr_points.at(polygon->ring->ring_prim->vertex_ptr()->point_ptr()));
                        ...
                        std::vector<SketchPointPtr>& new_line_points)
{
    ...
    for(auto& pos: m_key_lines)
    {
        // box intersect  precessor?
        SketchLinePtr line = pos.second;
...
void GeoData::_delete_nous_ring(SketchRingPtr ring)
{
    ...
    m_key_rings.erase(GeoData::ring_key(topo));
...
bool GeoData::_update_render_resource()
{
    ...
    for(auto& adj_info: m_ring_adj_polygons)
    {
        SketchPolygonPtr skp = adj_info.second.polygon;
        const nous::vec3 skp_normal = skp->normal;
        nous::quat rotate_to_xy_plane = nous::quat::FromTwoVectors(skp_normal, nous::vec3(0.f, 0.f, 1.f));
        ...
        for(auto& tri: skp->triangles)
        {
            auto fill_tri = [&position, &normal, &color, &uv, &prim_point_vertex_map, &mesh_point_vertex_map, &polygon_submesh, &skp,
                             &added_positions, &added_triangles,
                             rotate_to_xy_plane](std::pair<SketchPointPtr, nous::vec3>& point, auto& handle) -> size_t {
                size_t pt_idx = 0;
                if(point.first)
                {
                    ...
                    if(prim_point_vertex_map.find(point.first) == prim_point_vertex_map.end())
                    {
                        ...
                        uv.push_back(xoy_pos.x() / 100.0);
                        uv.push_back(xoy_pos.y() / 100.0);
    ...
    {
        submeshes.emplace_back("Polygon", triangles_index.size(), ele.second.size(), SubmeshData::Topology::Triangles, ele.first);
...
void GeoData::save(SketchSerializer& serializer)
{
    ...
    for(auto group: m_ele_groups)
    {
        ...
        if(group->type() == EElementType::NoBranchLineGroup)
        {
            serializer.begin_write_array("weld_lines");
            for(auto line: group->elements())
            {
                serializer.write(line->to_line()->line_prim->idx(), "idx");
            }
            serializer.end_write_array();
        }
        serializer.end_write_object();
...
void GeoData::load(SketchSerializer& serializer)
{
    ...
    for(int i = 0; i < num_prims; ++i)
    {
        ...
        if(is_line(current_prim))
        {
            ...
            {
                nous::Point* p1 = m_nous_mesh->point(prim_vertex->next_vertex_ptr()->point());
    ...
    for(int i = 0; i < weld_line_count; ++i)
    {
        ...
        serializer.begin_read_object();
        size_t line_count;
        serializer.begin_read_array("weld_lines", line_count);
        for(int j = 0; j < line_count; ++j)
        {
            nous::index_t line_idx;
            serializer.read(line_idx, "idx");
