...
namespace UI
{
    ...
    internal sealed class Floaty : Form, IFloaty
    {
        ...
        private ToolStripPanel _stripPanel;
        ...
        private void InitializeComponent()
        {
            ...
            this._stripPanel = new ToolStripPanel();
            this.Controls.Add(_stripPanel);
        }
        ...
        private void MakeFloatable(DockState dockState, int offsetx, int offsety)
        {
            ...
            Size fsz = new Size(sz.Width, sz.Height);
            if (_dockState.Container.Equals(_dockState.Handle))
            {
                if (fsz.Width < fsz.Height) fsz.Width = fsz.Height;
                fsz.Width += 18;
                fsz.Height = 70;
            }
            //if (fsz.Width > 600) 
            ...
            _dockState.Container.Parent = this._stripPanel;
            ...
            if (_dockState.Splitter != null)
            {
                ...
                _dockState.Splitter.Parent = this._stripPanel;
            }
            ...
            this.Bounds = new Rectangle(ps, fsz);
            _isFloating = true;
            Show();
        ...
        internal void Attach(DockState dockState)
        {
            ...
            (_dockState.Handle as CustomToolBar).BeginDrag += new EventHandler(Handle_MouseHover);
            (_dockState.Handle as CustomToolBar).EndDrag += new EventHandler(Handle_MouseLeave);
        }
        ...
        void Handle_MouseHover(object sender, EventArgs e)
        {
            ...
            System.Console.WriteLine("Handle_MouseHover - (" + _dockState.Handle.Location.X + ")");
            var topPanel = (_dockState.OrgDockHost as MainForm).barRoot.TopToolStripPanel;
            var bottomPanel = (_dockState.OrgDockHost as MainForm).barRoot.BottomToolStripPanel;
            if (bottomPanel.Location.Y - _dockState.Handle.Location.Y > _dockState.Handle.Location.Y)
                _dockState.Handle.Parent = topPanel;
            else
                _dockState.Handle.Parent = bottomPanel;
            _startFloating = true;
        }
        ...
        {
            Point pt = Cursor.Position;
            Point ps = _dockState.Handle.Location;
            //var distance = Math.Abs(Math.Sqrt((Math.Pow(pt.X - ps.X, 2) + Math.Pow(pt.Y - ps.Y, 2))));


            var topPanel = (_dockState.OrgDockHost as MainForm).barRoot.TopToolStripPanel;
            var bottomPanel = (_dockState.OrgDockHost as MainForm).barRoot.BottomToolStripPanel;
            var leftPanel = (_dockState.OrgDockHost as MainForm).barRoot.LeftToolStripPanel;
            var rightPanel = (_dockState.OrgDockHost as MainForm).barRoot.RightToolStripPanel;

            //System.Console.WriteLine("topPanel index: " + topPanel.Controls.IndexOf(_dockState.Handle));
            //System.Console.WriteLine("bottomPanel index: " + bottomPanel.Controls.IndexOf(_dockState.Handle));
            //System.Console.WriteLine("leftPanel index: " + leftPanel.Controls.IndexOf(_dockState.Handle));
            //System.Console.WriteLine("rightPanel index: " + rightPanel.Controls.IndexOf(_dockState.Handle));
            
            System.Console.WriteLine("topPanel index: " + topPanel.Bounds.Contains(topPanel.PointToClient(pt)));
            System.Console.WriteLine("bottomPanel index: " + bottomPanel.Bounds.Contains(bottomPanel.PointToClient(pt)));
            System.Console.WriteLine("leftPanel index: " + leftPanel.Bounds.Contains(leftPanel.PointToClient(pt)));
            System.Console.WriteLine("rightPanel index: " + rightPanel.Bounds.Contains(rightPanel.PointToClient(pt)));

            if (topPanel.Bounds.Contains(topPanel.PointToClient(pt)) == false &&
                bottomPanel.Bounds.Contains(bottomPanel.PointToClient(pt)) == false &&
                leftPanel.Bounds.Contains(leftPanel.PointToClient(pt)) == false &&
                rightPanel.Bounds.Contains(rightPanel.PointToClient(pt)) == false)
                MakeFloatable(_dockState, pt.X, pt.Y);
            else
                this.Hide();
