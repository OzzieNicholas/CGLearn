...
namespace UI
{
    public partial class Form_Move_ByRotate: UI.ParaForm
    {
        public static Form_Move_ByRotate m_instance = new Form_Move_ByRotate();
        
        public static Form_Move_ByRotate instance => m_instance;

        public Form_Move_ByRotate()
        {
            InitializeComponent();
        ...
        private void btn_move_Click(object sender, EventArgs e)
        {
            if (Native.curObjIds().Length == 0 && Native.focusedViewIds().Length > 0) { //移动视图平面
                if (MessageBox.Show("没有选中物体，是否移动视图平面?", "提示", MessageBoxButtons.OKCancel) == DialogResult.OK) {
                    Native.moveFocusedViewsPlaneByRotateInWorkSpace(tb_x0.Value, tb_y0.Value, tb_z0.Value, (Define.SpaceAxis)cbo_axis.SelectedKey, tb_angl.Value);
                }
            }
            else { //移动物体
                var res = Native.moveCurObjsByRotateInWorkSpace(tb_x0.Value, tb_y0.Value, tb_z0.Value, (Define.SpaceAxis)cbo_axis.SelectedKey, tb_angl.Value);
