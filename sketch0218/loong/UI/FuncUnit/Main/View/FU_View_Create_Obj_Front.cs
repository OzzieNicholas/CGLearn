...
namespace UI
{
    class FU_View_Create_Obj_Front : FU_Main
    {
        public FU_View_Create_Obj_Front() {
            m_name = "view_create_obj_front";
            m_group = "view";
        ...
        public override void onClick(object sender, EventArgs e, Keys ctrlKeys, DObject paras) {
            ...
            if (paras != null && paras.ContainsKey("pick_part")) {
                Native.activateController("Create_View_Part", "mode", "front");
            }
            else if (paras != null && paras.ContainsKey("pick_comp")) {
                Native.activateController("Create_View_Comp", "mode", "front");
            }
            else {
                ...
                string name = (paras != null && paras.ContainsKey("name")) ? paras.GetString("name") : "front";

                if (bWorld)
                    Native.createObjViewFromModelSide(ids[0], name, Define.ObjectSide.SIDE_FRONT, bTemp);
                else
                    Native.createObjViewFromObjSide(ids[0], name, Define.ObjectSide.SIDE_FRONT, bTemp);
            }
        }
    }
}
