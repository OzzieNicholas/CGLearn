...
{
    [ToolboxItem(false)]
    public class ToolBarPanelTop : ToolBarPanel
    {
        public ToolBarPanelTop() {
            m_barLayout = ToolStripLayoutStyle.HorizontalStackWithOverflow;
            m_panelPosition = CustomBarsProvision.BAR_LOCATION_TOP;
        }
            ...
            for (var l = 0; l < m_bars.Count; l++) {
                ...
                for (int b = 0; b < line.Count; b++) {
                    ...
                    if (barRect.IntersectsWith(insertRect)
                        && barRect.Bottom - insertRect.Top >= barRect.Height / 2) {
                        if ((float)(insertRect.Left - barRect.Left) / (float)barRect.Width < 0.5) //bar的前面
                            insertIdx = b;
                        else //bar的后面
                            insertIdx = b + 1;
                ...
                if (line.Count > 0) {
                    ...
                    if (barRect.Bottom - insertRect.Top >= barRect.Height / 2) {
                        insertLine = l;
