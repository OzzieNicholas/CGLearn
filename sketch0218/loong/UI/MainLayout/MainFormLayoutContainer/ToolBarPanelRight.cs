...
{
    [ToolboxItem(false)]
    public class ToolBarPanelRight : ToolBarPanel
    {
        public ToolBarPanelRight() {
            m_barLayout = ToolStripLayoutStyle.VerticalStackWithOverflow;
            m_panelPosition = CustomBarsProvision.BAR_LOCATION_RIGHT;
        }
        ...
        public override bool indexOf(Rectangle insertRect, out int insertLine, out int insertIdx) {
            ...
            int x = 1;
            for (int l = m_bars.Count - 1; l >= 0; --l) {
                var line = m_bars[l];
                int y = 0;
                int w = 0;
                ...
                for (int b = 0; b < line.Count; b++) {
                    ...
                    if (barRect.IntersectsWith(insertRect) &&
                        (insertRect.Right - barRect.Left >= barRect.Width / 2)) {
                ...
                if (line.Count > 0) {
                    ...
                    if (insertRect.Right - barRect.Left >= barRect.Width / 2) {
                        insertLine = l;
        ...
        public override void relayout(Rectangle reserve) {
            ...
            this.SetBounds(this.Parent.Width - x, this.Location.Y, x, this.Height);
            this.ResumeLayout();
        }
    }
}
