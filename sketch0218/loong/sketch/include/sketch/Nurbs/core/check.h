 ...
 */
inline bool isValidRelation(unsigned int degree, size_t num_knots, size_t num_ctrl_pts) { return (num_knots - degree - 1) == num_ctrl_pts; }
 ...
 */
template<typename T> bool isKnotVectorMonotonic(const std::vector<T>& knots) { return std::is_sorted(knots.begin(), knots.end()); }
 ...
 */
template<typename T> bool curveIsValid(unsigned int degree, const std::vector<T>& knots, const std::vector<NurbsPointPtr<T>>& control_points)
{
 ...
 */
template<typename T>
bool curveIsValid(unsigned int degree, const std::vector<T>& knots, const std::vector<NurbsPointPtr<T>>& control_points,
...
bool surfaceIsValid(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u, const std::vector<T>& knots_v,
                    const array2<NurbsPointPtr<T>>& control_points)
{
    if(degree_u < 1 || degree_u > 9 || degree_v < 1 || degree_v > 9)
    {
        return false;
    }
    if(!isValidRelation(degree_u, knots_u.size(), control_points.rows()) || !isValidRelation(degree_v, knots_v.size(), control_points.cols()))
    {
        return false;
    }
...
template<typename T>
bool surfaceIsValid(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u, const std::vector<T>& knots_v,
                    const array2<NurbsPointPtr<T>>& control_points, const array2<T>& weights)
{
...
template<typename T> bool isKnotVectorClosed(unsigned int degree, const std::vector<T>& knots)
{
    for(int i = 0; i < degree - 1; ++i)
    {
        ...
        if(std::abs((knots[i + 1] - knots[i]) - (knots[j + 1] - knots[j])) > std::numeric_limits<T>::epsilon())
        {
            return false;
        }
    }
...
template<typename T> bool isArray1Closed(unsigned int degree, const std::vector<T>& vec)
{
    for(int i = 0; i < degree; ++i)
    {
        ...
        if constexpr(std::is_same_v<T, NurbsPointPtr<float>> || std::is_same_v<T, NurbsPointPtr<double>>)
        {
            if((vec[i]->m_point - vec[j]->m_point).norm() > 1e-2)
            {
                return false;
            }
        }
...
// template<>
// bool isArray1Closed(unsigned int degree, const std::vector<NurbsPointPtr<float>>& vec)
//{
//     for(int i = 0; i < degree; ++i)
//     {
//         int j = vec.size() - degree + i;
//         if((vec[i]->m_point - vec[j]->m_point) > 1e-2)
//         {
//             return false;
//         }
//     }
...
template<typename T> bool isArray2ClosedU(unsigned int degree_u, const array2<T>& arr)
{
    for(int i = 0; i < degree_u; ++i)
    {
        ...
        {
            int k = arr.rows() - degree_u + i;

            if constexpr(std::is_same_v<T, NurbsPointPtr<float>> || std::is_same_v<T, NurbsPointPtr<double>>)
            {
                if((arr(i, j)->m_point - arr(k, j)->m_point).norm() > 1e-2)
                {
                    return false;
                }
            }
...
template<typename T> bool isArray2ClosedV(unsigned int degree_v, const array2<T>& arr)
{
    for(int i = 0; i < arr.rows(); ++i)
    {
        ...
        {
            int k = arr.cols() - degree_v + j;
            if constexpr(std::is_same_v<T, NurbsPointPtr<float>> || std::is_same_v<T, NurbsPointPtr<double>>)
            {
                if((arr(i, j)->m_point - arr(i, k)->m_point).norm() > 1e-2)
                {
                    return false;
                }
            }
 ...
 * @return Whether valid
 */
template<typename T> bool curveIsValid(const Curve<T>& crv) { return internal::curveIsValid(crv.degree, crv.knots, crv.control_points); }
...
{
    return internal::surfaceIsValid(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points);
}
...
{
    return internal::surfaceIsValid(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, srf.weights);
}
...
{
    return internal::isArray1Closed(crv.degree, crv.control_points) && internal::isKnotVectorClosed(crv.degree, crv.knots);
}
...
{
    return internal::isArray1Closed(crv.degree, crv.control_points) && internal::isArray1Closed(crv.degree, crv.weights) &&
...
{
    return internal::isArray2ClosedU(srf.degree_u, srf.control_points) && internal::isKnotVectorClosed(srf.degree_u, srf.knots_u);
}
...
{
    return internal::isArray2ClosedV(srf.degree_v, srf.control_points) && internal::isKnotVectorClosed(srf.degree_v, srf.knots_v);
}
