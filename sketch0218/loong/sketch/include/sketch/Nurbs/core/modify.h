 ...
 */
template<typename T>
void curveKnotInsert(unsigned int deg, const std::vector<T>& knots, const std::vector<NurbsPointPtr<T>>& cp, T u, unsigned int r,
                     std::vector<T>& new_knots, std::vector<NurbsPointPtr<T>>& new_cp)
{
    ...
    new_cp.resize(cp.size() + r, nullptr);
    ...
    {
        new_cp[i + r] = cp[i]->clone();
    }

    // Copy affected control points
    std::vector<Eigen::Vector<T, 3>> tmp;
    tmp.resize(deg - s + 1);
    for(int i = 0; i < deg - s + 1; ++i)
    {
        tmp[i] = cp[k - deg + i]->m_point;
    }
    ...
    for(int j = 1; j <= r; ++j)
    {
        ...
        new_cp[L] = std::make_shared<NurbsPoint<T>>(tmp[0]);
        new_cp[k + r - j - s] = std::make_shared<NurbsPoint<T>>(tmp[deg - j - s]);
    }
    int L = k - deg + r;
    for(int i = L + 1; i < k - s; ++i)
    {
        new_cp[i] = std::make_shared<NurbsPoint<T>>(tmp[i - L]);
    }
}

template<int dim, typename T>
void curveKnotInsert(unsigned int deg, const std::vector<T>& knots, const std::vector<Eigen::Vector<T, dim>>& cp, T u, unsigned int r,
                     std::vector<T>& new_knots, std::vector<Eigen::Vector<T, dim>>& new_cp)
{
    int k = findSpan(deg, knots, u);
    unsigned int s = knotMultiplicity(knots, u);
    assert(s <= deg);// Multiplicity cannot be greater than degree
    ...
    // Copy affected control points
    std::vector<Eigen::Vector<T, dim>> tmp;
    tmp.resize(deg - s + 1);
    for(int i = 0; i < deg - s + 1; ++i)
    {
        tmp[i] = cp[k - deg + i];
 ...
 */
template<typename T>
void surfaceKnotInsert(unsigned int degree, const std::vector<T>& knots, const array2<NurbsPointPtr<T>>& cp, T knot, unsigned int r, bool along_u,
                       std::vector<T>& new_knots, array2<NurbsPointPtr<T>>& new_cp)
{
    ...
    if(s == degree)
    {
        new_cp.resize(cp.rows(), cp.cols(), nullptr);
        for(int i = 0; i < new_cp.rows(); ++i)
        {
            for(int j = 0; j < new_cp.cols(); ++j)
            {
    ...
    std::vector<Eigen::Vector<T, 3>> tmp(degree + 1);

    if(along_u)
    {
        // Create new control points with additional rows
        new_cp.resize(cp.rows() + r, cp.cols(), nullptr);
        ...
        for(int col = 0; col < cp.cols(); ++col)
        {
            ...
            for(int j = 1; j <= r; ++j)
            {
                ...
                new_cp(L, col) = std::make_shared<NurbsPoint<T>>(tmp[0]);
                new_cp(span + r - j - s, col) = std::make_shared<NurbsPoint<T>>(tmp[degree - j - s]);
            }

            int L = span - degree + r;
            for(int i = L + 1; i < span - s; ++i)
            {
                new_cp(i, col) = std::make_shared<NurbsPoint<T>>(tmp[i - L]);
            }
        }
    }
    else
    {
        ...
        new_cp.resize(cp.rows(), cp.cols() + r, nullptr);
        ...
        for(int row = 0; row < cp.rows(); ++row)
        {
            ...
            for(int i = 0; i <= span - degree; ++i)
            {
                new_cp(row, i) = cp(row, i)->clone();
            }
            for(int i = span - s; i < cp.cols(); ++i)
            {
                new_cp(row, i + r) = cp(row, i)->clone();
            }

            // Copy affected control points to temp array
            for(int i = 0; i < degree - s + 1; ++i)
            {
                tmp[i] = cp(row, span - degree + i)->m_point;
            }
            ...
            for(int j = 1; j <= r; ++j)
            {
                ...
                new_cp(row, L) = std::make_shared<NurbsPoint<T>>(tmp[0]);
                new_cp(row, span + r - j - s) = std::make_shared<NurbsPoint<T>>(tmp[degree - j - s]);
            }
            int L = span - degree + r;
            for(int i = L + 1; i < span - s; ++i)
            {
                new_cp(row, i) = std::make_shared<NurbsPoint<T>>(tmp[i - L]);
            }
        }
    }
}
...
template<int dim, typename T>
void surfaceKnotInsert(unsigned int degree, const std::vector<T>& knots, const array2<Eigen::Vector<T, dim>>& cp, T knot, unsigned int r,
                       bool along_u, std::vector<T>& new_knots, array2<Eigen::Vector<T, dim>>& new_cp)
{
    int span = findSpan(degree, knots, knot);
    unsigned int s = knotMultiplicity(knots, knot);
    assert(s <= degree);// Knot multiplicity cannot be greater than degree
    ...
    // Create a temporary container for affected control points per row/column
    std::vector<Eigen::Vector<T, dim>> tmp(degree + 1);
 ...
 */
template<typename T>
void curveSplit(unsigned int degree, const std::vector<T>& knots, const std::vector<NurbsPointPtr<T>>& control_points, T u,
                std::vector<T>& left_knots, std::vector<NurbsPointPtr<T>>& left_control_points, std::vector<T>& right_knots,
                std::vector<NurbsPointPtr<T>>& right_control_points)
{
    std::vector<T> tmp_knots;
    std::vector<Eigen::Vector<T, 3>> tmp_cp;
...
template<int dim, typename T>
void curveSplit(unsigned int degree, const std::vector<T>& knots, const std::vector<Eigen::Vector<T, dim>>& control_points, T u,
                std::vector<T>& left_knots, std::vector<Eigen::Vector<T, dim>>& left_control_points, std::vector<T>& right_knots,
                std::vector<Eigen::Vector<T, dim>>& right_control_points)
{
    std::vector<T> tmp_knots;
    std::vector<Eigen::Vector<T, dim>> tmp_cp;
 ...
 */
template<typename T>
void surfaceSplit(unsigned int degree, const std::vector<T>& knots, const array2<NurbsPointPtr<T>>& control_points, T param, bool along_u,
                  std::vector<T>& left_knots, array2<NurbsPointPtr<T>>& left_control_points, std::vector<T>& right_knots,
                  array2<NurbsPointPtr<T>>& right_control_points)
{
    std::vector<T> tmp_knots;
    array2<Eigen::Vector<T, 3>> tmp_cp;

    int span = findSpan(degree, knots, param);
    unsigned int r = degree - knotMultiplicity(knots, param);
    internal::surfaceKnotInsert(degree, knots, control_points, param, r, along_u, tmp_knots, tmp_cp);
...
template<int dim, typename T>
void surfaceSplit(unsigned int degree, const std::vector<T>& knots, const array2<Eigen::Vector<T, dim>>& control_points, T param, bool along_u,
                  std::vector<T>& left_knots, array2<Eigen::Vector<T, dim>>& left_control_points, std::vector<T>& right_knots,
                  array2<Eigen::Vector<T, dim>>& right_control_points)
{
    std::vector<T> tmp_knots;
    array2<Eigen::Vector<T, dim>> tmp_cp;
...
template<typename T> Curve<T> curveKnotInsert(const Curve<T>& crv, T u, unsigned int repeat = 1)
{
    ...
    internal::curveKnotInsert(crv.degree, crv.knots, crv.control_points, u, repeat, new_crv.knots, new_crv.control_points);
    return new_crv;
}
 ...
 */
template<typename T> RationalCurve<T> curveKnotInsert(const RationalCurve<T>& crv, T u, unsigned int repeat = 1)
{
    RationalCurve<T> new_crv;
    new_crv.degree = crv.degree;

    // Convert to homogenous coordinates
    std::vector<Eigen::Vector<T, 4>> Cw;
    Cw.reserve(crv.control_points.size());
    for(int i = 0; i < crv.control_points.size(); ++i)
    {
        Cw.push_back(util::cartesianToHomogenous(crv.control_points[i]->m_point, crv.weights[i]));
    }

    // Perform knot insertion and get new knots and control points
    std::vector<Eigen::Vector<T, 4>> new_Cw;
    ...
    {
        new_crv.control_points.push_back(std::make_shared<NurbsPoint<T>>(util::homogenousToCartesian(new_Cw[i])));
 ...
 */
template<typename T> Surface<T> surfaceKnotInsertU(const Surface<T>& srf, T u, unsigned int repeat = 1)
{
    Surface<T> new_srf;
    new_srf.degree_u = srf.degree_u;
    new_srf.degree_v = srf.degree_v;
    new_srf.knots_v = srf.knots_v;
    internal::surfaceKnotInsert(new_srf.degree_u, srf.knots_u, srf.control_points, u, repeat, true, new_srf.knots_u, new_srf.control_points);
    return new_srf;
}
 ...
 */
template<typename T> RationalSurface<T> surfaceKnotInsertU(const RationalSurface<T>& srf, T u, unsigned int repeat = 1)
{
    ...
    array2<Eigen::Vector<T, 4>> Cw(srf.control_points.rows(), srf.control_points.cols());
    for(int i = 0; i < srf.control_points.rows(); ++i)
    {
        for(int j = 0; j < srf.control_points.cols(); ++j)
        {
            Cw(i, j) = util::cartesianToHomogenous(srf.control_points(i, j)->m_point, srf.weights(i, j));
        }
    }

    // New knots and new homogenous control points after knot insertion
    std::vector<T> new_knots_u;
    array2<Eigen::Vector<T, 4>> new_Cw;
    internal::surfaceKnotInsert(srf.degree_u, srf.knots_u, Cw, u, repeat, true, new_srf.knots_u, new_Cw);
    ...
    for(int i = 0; i < new_Cw.rows(); ++i)
    {
        ...
        {
            new_srf.control_points(i, j) = std::make_shared<NurbsPoint<T>>(util::homogenousToCartesian(new_Cw(i, j)));
 ...
 * @return New Surface object after knot insertion
 */
template<typename T> Surface<T> surfaceKnotInsertV(const Surface<T>& srf, T v, unsigned int repeat = 1)
{
    Surface<T> new_srf;
    new_srf.degree_u = srf.degree_u;
    new_srf.degree_v = srf.degree_v;
    ...
    internal::surfaceKnotInsert(srf.degree_v, srf.knots_v, srf.control_points, v, repeat, false, new_srf.knots_v, new_srf.control_points);
    return new_srf;
}
 ...
 * @return New RationalSurface object after knot insertion
 */
template<typename T> RationalSurface<T> surfaceKnotInsertV(const RationalSurface<T>& srf, T v, unsigned int repeat = 1)
{
    RationalSurface<T> new_srf;
    new_srf.degree_u = srf.degree_u;
    new_srf.degree_v = srf.degree_v;
    ...
    array2<Eigen::Vector<T, 4>> new_Cw;
    internal::surfaceKnotInsert(srf.degree_v, srf.knots_v, Cw, v, repeat, false, new_srf.knots_v, new_Cw);
...
template<typename T> std::tuple<Curve<T>, Curve<T>> curveSplit(const Curve<T>& crv, T u)
{
    ...
    internal::curveSplit(crv.degree, crv.knots, crv.control_points, u, left.knots, left.control_points, right.knots, right.control_points);
 ...
 * @return Tuple with first half and second half of the curve
 */
template<typename T> std::tuple<RationalCurve<T>, RationalCurve<T>> curveSplit(const RationalCurve<T>& crv, T u)
{
    RationalCurve<T> left, right;
    left.degree = crv.degree;
    right.degree = crv.degree;

    std::vector<Eigen::Vector<T, 4>> Cw, left_Cw, right_Cw;
    Cw.reserve(crv.control_points.size());
    for(int i = 0; i < crv.control_points.size(); ++i)
    {
        Cw.push_back(util::cartesianToHomogenous(crv.control_points[i]->m_point, crv.weights[i]));
    ...
    {
        left.control_points.push_back(std::make_shared<NurbsPoint<T>>(util::homogenousToCartesian(left_Cw[i])));
        left.weights.push_back(left_Cw[i].w());
    }
    for(int i = 0; i < right_Cw.size(); ++i)
    {
        right.control_points.push_back(std::make_shared<NurbsPoint<T>>(util::homogenousToCartesian(right_Cw[i])));
...
template<typename T> std::tuple<Surface<T>, Surface<T>> surfaceSplitU(const Surface<T>& srf, T u)
{
    ...
    internal::surfaceSplit(srf.degree_u, srf.knots_u, srf.control_points, u, true, left.knots_u, left.control_points, right.knots_u,
 ...
 * @return Tuple with first and second half of the surfaces
 */
template<typename T> std::tuple<RationalSurface<T>, RationalSurface<T>> surfaceSplitU(const RationalSurface<T>& srf, T u)
{
    ...
    array2<Eigen::Vector<4, T>> Cw = util::cartesianToHomogenous(srf.control_points, srf.weights);

    // Split surface with homogenous coordinates
    array2<Eigen::Vector<4, T>> left_Cw, right_Cw;
    internal::surfaceSplit(srf.degree_u, srf.knots_u, Cw, u, true, left.knots_u, left_Cw, right.knots_u, right_Cw);
...
template<typename T> std::tuple<Surface<T>, Surface<T>> surfaceSplitV(const Surface<T>& srf, T v)
{
    ...
    internal::surfaceSplit(srf.degree_v, srf.knots_v, srf.control_points, v, false, left.knots_v, left.control_points, right.knots_v,
                           right.control_points);
    return std::make_tuple(std::move(left), std::move(right));
}
 ...
 * @return Tuple with first and second half of the surfaces
 */
template<typename T> std::tuple<RationalSurface<T>, RationalSurface<T>> surfaceSplitV(const RationalSurface<T>& srf, T v)
{
    RationalSurface<T> left, right;
    left.degree_u = srf.degree_u;
    left.degree_v = srf.degree_v;
    ...
    // Split surface with homogenous coordinates
    array2<Eigen::Vector<4, T>> left_Cw, right_Cw;
    internal::surfaceSplit(srf.degree_v, srf.knots_v, Cw, v, false, left.knots_v, left_Cw, right.knots_v, right_Cw);
