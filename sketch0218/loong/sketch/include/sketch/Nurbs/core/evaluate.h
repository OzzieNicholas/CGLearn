...
Eigen::Vector<T, dim> curvePoint(unsigned int degree, const std::vector<T>& knots, const std::vector<Eigen::Vector<T, dim>>& control_points, T u)
{
    // Initialize result to 0s
    Eigen::Vector<T, dim> point = Eigen::Vector<T, dim>::Zero();
...
template<int dim, typename T>
Eigen::Vector<T, dim> curvePoint(unsigned int degree, const std::vector<T>& knots, const std::vector<NurbsPointPtr<T>>& control_points, T u)
{
    // Initialize result to 0s
    Eigen::Vector<T, dim> point = Eigen::Vector<T, dim>::Zero();
    ...
    for(unsigned int j = 0; j <= degree; j++)
    {
        point += static_cast<T>(N[j]) * control_points[span - degree + j]->m_point;
    }
    return point;
}
 ...
 */
template<int dim, typename T>
std::vector<Eigen::Vector<T, dim>> curveDerivatives(unsigned int degree, const std::vector<T>& knots,
                                                    const std::vector<Eigen::Vector<T, dim>>& control_points, int num_ders, T u)
{

    typedef Eigen::Vector<T, dim> tvecn;
    using std::vector;

    std::vector<Eigen::Vector<T, dim>> curve_ders;
    curve_ders.resize(num_ders + 1, Eigen::Vector<T, dim>::Zero());

    // Assign higher order derivatives to zero
    for(int k = degree + 1; k <= num_ders; k++)
    {
        curve_ders[k] = tvecn::Zero();
    }
...
template<int dim, typename T>
std::vector<Eigen::Vector<T, dim>> curveDerivatives(unsigned int degree, const std::vector<T>& knots,
                                                    const std::vector<NurbsPointPtr<T>>& control_points, int num_ders, T u)
{
    ...
    for(int k = 0; k <= du; k++)
    {
        ...
        for(int j = 0; j <= static_cast<int>(degree); j++)
        {
            curve_ders[k] += static_cast<T>(ders(k, j)) * control_points[span - degree + j]->m_point;
        }
    }
    return curve_ders;
}
 ...
 */
template<int dim, typename T>
Eigen::Vector<T, dim> surfacePoint(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u, const std::vector<T>& knots_v,
                                   const array2<Eigen::Vector<T, dim>>& control_points, T u, T v)
{
    ...
    {
        Eigen::Vector<T, dim> temp = Eigen::Vector<T, dim>::Zero();
        for(int k = 0; k <= degree_u; k++)
        {
            temp += static_cast<T>(Nu[k]) * control_points(span_u - degree_u + k, span_v - degree_v + l);
        }
...
template<int dim, typename T>
Eigen::Vector<T, dim> surfacePoint(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u, const std::vector<T>& knots_v,
                                   const array2<NurbsPointPtr<T>>& control_points, T u, T v)
{
    ...
    for(int l = 0; l <= degree_v; l++)
    {
        ...
        for(int k = 0; k <= degree_u; k++)
        {
            temp += static_cast<T>(Nu[k]) * control_points(span_u - degree_u + k, span_v - degree_v + l)->m_point;
        }
 ...
 */
template<int dim, typename T>
array2<Eigen::Vector<T, dim>> surfaceDerivatives(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u,
                                                 const std::vector<T>& knots_v, const array2<Eigen::Vector<T, dim>>& control_points,
                                                 unsigned int num_ders, T u, T v)
{

    array2<Eigen::Vector<T, dim>> surf_ders(num_ders + 1, num_ders + 1, Eigen::Vector<T, dim>::Zero());
    ...
    for(int k = degree_u + 1; k <= num_ders; k++)
    {
        ...
        {
            surf_ders(k, l) = Eigen::Vector<T, dim>::Zero();
        }
    }
    ...
    for(int k = 0; k <= du; k++)
    {
        ...
        {
            temp[s] = Eigen::Vector<T, dim>::Zero();
            for(int r = 0; r <= degree_u; r++)
            {
                temp[s] += static_cast<T>(ders_u(k, r)) * control_points(span_u - degree_u + r, span_v - degree_v + s);
            }
        }
...
template<int dim, typename T>
array2<Eigen::Vector<T, dim>> surfaceDerivatives(unsigned int degree_u, unsigned int degree_v, const std::vector<T>& knots_u,
                                                 const std::vector<T>& knots_v, const array2<NurbsPointPtr<T>>& control_points, unsigned int num_ders,
                                                 T u, T v)
{
    ...
    for(int k = 0; k <= du; k++)
    {
        for(int s = 0; s <= degree_v; s++)
        {
            ...
            for(int r = 0; r <= degree_u; r++)
            {
                temp[s] += static_cast<T>(ders_u(k, r)) * control_points(span_u - degree_u + r, span_v - degree_v + s)->m_point;
            }
        }
...
@return point Resulting point on the curve at parameter u.
*/
template<typename T> Eigen::Vector<T, 3> curvePoint(const Curve<T>& crv, T u)
{
 ...
 */
template<typename T> Eigen::Vector<T, 3> curvePoint(const RationalCurve<T>& crv, T u)
{

    typedef Eigen::Vector<T, 4> tvecnp1;
    ...
    {
        tvecnp1 wp = util::cartesianToHomogenous(crv.control_points[i]->m_point, crv.weights[i]);
        Cw.push_back(wp);
    }
 ...
 * E.g. curve_ders[n] is the nth derivative at u, where 0 <= n <= num_ders.
 */
template<typename T> std::vector<Eigen::Vector<T, 3>> curveDerivatives(const Curve<T>& crv, int num_ders, T u)
{
 ...
 * num_ders-1.
 */
template<typename T> std::vector<Eigen::Vector<T, 3>> curveDerivatives(const RationalCurve<T>& crv, int num_ders, T u)
{

    typedef Eigen::Vector<T, 3> tvecn;
    typedef Eigen::Vector<T, 4> tvecnp1;
 ...
 */
template<typename T> Eigen::Vector<T, 3> curveTangent(const Curve<T>& crv, T u)
{
    std::vector<Eigen::Vector<T, 3>> ders = curveDerivatives(crv, 1, u);
    Eigen::Vector<T, 3> du = ders[1];
    T du_len = du.norm();
 ...
 * @return Unit tangent of the curve at u.
 */
template<typename T> Eigen::Vector<T, 3> curveTangent(const RationalCurve<T>& crv, T u)
{
    std::vector<Eigen::Vector<T, 3>> ders = curveDerivatives(crv, 1, u);
    Eigen::Vector<T, 3> du = ders[1];
    T du_len = du.norm();
 ...
 */
template<typename T> Eigen::Vector<T, 3> surfacePoint(const Surface<T>& srf, T u, T v)
{
    return internal::surfacePoint(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, u, v);
}
 ...
 * @return Resulting point on the surface at (u, v).
 */
template<typename T> Eigen::Vector<T, 3> surfacePoint(const RationalSurface<T>& srf, T u, T v)
{
    ...
    for(int i = 0; i < srf.control_points.rows(); i++)
    {
        ...
        {
            tvecnp1 wp = util::cartesianToHomogenous(srf.control_points(i, j)->m_point, srf.weights(i, j));
            Cw(i, j) = wp;
        }
    }

    // Compute point using homogenous coordinates
    tvecnp1 pointw = internal::surfacePoint(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, Cw, u, v);
 ...
 */
template<typename T> array2<Eigen::Vector<T, 3>> surfaceDerivatives(const Surface<T>& srf, int num_ders, T u, T v)
{
    return internal::surfaceDerivatives(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, srf.control_points, num_ders, u, v);
}
 ...
 */
template<typename T> array2<Eigen::Vector<T, 3>> surfaceDerivatives(const RationalSurface<T>& srf, int num_ders, T u, T v)
{
    ...
    homo_cp.resize(srf.control_points.rows(), srf.control_points.cols(), tvecnp1::Zero());
    for(int i = 0; i < srf.control_points.rows(); ++i)
    {
        for(int j = 0; j < srf.control_points.cols(); ++j)
        {
            homo_cp(i, j) = util::cartesianToHomogenous(srf.control_points(i, j)->m_point, srf.weights(i, j));
        }
    }

    array2<tvecnp1> homo_ders = internal::surfaceDerivatives(srf.degree_u, srf.degree_v, srf.knots_u, srf.knots_v, homo_cp, num_ders, u, v);

    array2<tvecn> Aders;
    Aders.resize(num_ders + 1, num_ders + 1, tvecn::Zero());
    ...
    array2<tvecn> surf_ders(num_ders + 1, num_ders + 1, tvecn::Zero());
 ...
 */
template<typename T> std::tuple<Eigen::Vector<T, 3>, Eigen::Vector<T, 3>> surfaceTangent(const Surface<T>& srf, T u, T v)
{
    array2<Eigen::Vector<T, 3>> ptder = surfaceDerivatives(srf, 1, u, v);
    Eigen::Vector<T, 3> du = ptder(1, 0);
    Eigen::Vector<T, 3> dv = ptder(0, 1);
    T du_len = ptder(1, 0).norm();
    T dv_len = ptder(0, 1).norm();
    if(!util::close(du_len, T(0)))
    {
        du /= du_len;
    }
 ...
 * @return Tuple with unit tangents along u- and v-directions
 */
template<typename T> std::tuple<Eigen::Vector<T, 3>, Eigen::Vector<T, 3>> surfaceTangent(const RationalSurface<T>& srf, T u, T v)
{
    array2<Eigen::Vector<T, 3>> ptder = surfaceDerivatives(srf, 1, u, v);
    Eigen::Vector<T, 3> du = ptder(1, 0);
    Eigen::Vector<T, 3> dv = ptder(0, 1);
 ...
 */
template<typename T> Eigen::Vector<T, 3> surfaceNormal(const Surface<T>& srf, T u, T v)
{
    array2<Eigen::Vector<T, 3>> ptder = surfaceDerivatives(srf, 1, u, v);
    Eigen::Vector<T, 3> n = ptder(0, 1).cross(ptder(1, 0));
    T n_len = n.norm() if(!util::close(n_len, T(0))) { n /= n_len; }
    return n;
}
 ...
 */
template<typename T> Eigen::Vector<T, 3> surfaceNormal(const RationalSurface<T>& srf, T u, T v)
{
    array2<Eigen::Vector<T, 3>> ptder = surfaceDerivatives(srf, 1, u, v);
    Eigen::Vector<T, 3> n = ptder(0, 1).cross(ptder(1, 0));
    T n_len = n.norm();
