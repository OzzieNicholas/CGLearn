...
//
template<typename T> class SketchRTree;
...
public:
    ...
    void clear()
    {
        ...
        m_nousptr_points.clear();
        m_nousptr_lines.clear();
        m_nousptr_rings.clear();
    ...
    void get_point_adj_lines(SketchPointPtr point, std::unordered_set<SketchLinePtr>& adj_lines)
    {
        ...
        {
            // 这里有问题，先跳过了
            const size_t line_key = GeoData::line_key(point, np);
            // if(m_key_lines.find(line_key) != m_key_lines.end())
            //{
            //     adj_lines.insert(m_key_lines.at(line_key));
            // }
            adj_lines.insert(m_key_lines.at(line_key));
        }
    }
    ...
    void delete_polygon(SketchPolygonPtr polygon, bool need_delete_line = false,
                        bool need_merge_line = true, std::unordered_set<SketchPointPtr>* force_keeped_point_set = nullptr);
...
protected:
    ...
    std::shared_ptr<SketchRTree<SketchLinePtr>> m_lines_tree;
    std::shared_ptr<SketchRTree<SketchRingPtr>> m_ring_tree;
