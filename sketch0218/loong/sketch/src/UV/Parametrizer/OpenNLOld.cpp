...
#include "sketch/UV/Parametrizer/OpenNLOld.h"
...
/* SuperLU includes */
#include "sketch/UV/SuperLU/ssp_defs.h"
#include "sketch/UV/SuperLU/super_lu_util.h"
...
{
    fprintf(stderr, "OpenNLOLD assertion failed: %s, file:%s, line:%d\n", cond, file, line);
    abort();
}

static void __nlOLD_range_assertion_failed(float x, float min_val, float max_val, char* file, int line)
{
    fprintf(stderr, "OpenNLOLD range assertion failed: %f in [ %f ... %f ], file:%s, line:%d\n", x, min_val, max_val, file, line);
    abort();
}

static void __nlOLD_should_not_have_reached(char* file, int line)
{
    fprintf(stderr, "OpenNLOLD should not have reached this point: file:%s, line:%d\n", file, line);
    abort();
}
...
#define __nlOLD_range_assert(x, min_val, max_val)                                                                                                    \
    {                                                                                                                                                \
        ...
        {                                                                                                                                            \
            __nlOLD_range_assertion_failed(x, min_val, max_val, __FILE__, __LINE__);                                                                 \
        }                                                                                                                                            \
    }
...
#define __NLOLD_DELETE(x)                                                                                                                            \
    free(x);                                                                                                                                         \
    x = NULL
#define __NLOLD_DELETE_ARRAY(x)                                                                                                                      \
    free(x);                                                                                                                                         \
    x = NULL
...
static void __nlOLDRowColumnZero(__NLOLDRowColumn* c) { c->size = 0; }
...
static void __nlOLDSparseMatrixConstruct(__NLOLDSparseMatrix* M, NLOLDuint m, NLOLDuint n, NLOLDenum storage)
{
    NLOLDuint i;
    M->m = m;
    M->n = n;
...
static void __nlOLDSparseMatrixAdd(__NLOLDSparseMatrix* M, NLOLDuint i, NLOLDuint j, NLOLDfloat value)
{
...
static void __nlOLDSparseMatrix_mult_rows_symmetric(__NLOLDSparseMatrix* A, NLOLDfloat* x, NLOLDfloat* y)
{
    NLOLDuint m = A->m;
    NLOLDuint i, ij;
...
static void __nlOLDSparseMatrix_mult_rows(__NLOLDSparseMatrix* A, NLOLDfloat* x, NLOLDfloat* y)
{
    NLOLDuint m = A->m;
    NLOLDuint i, ij;
    __NLOLDRowColumn* Ri = NULL;
...
static void __nlOLDSparseMatrix_mult_cols_symmetric(__NLOLDSparseMatrix* A, NLOLDfloat* x, NLOLDfloat* y)
{
    NLOLDuint n = A->n;
    NLOLDuint j, ii;
...
static void __nlOLDSparseMatrix_mult_cols(__NLOLDSparseMatrix* A, NLOLDfloat* x, NLOLDfloat* y)
{
    NLOLDuint n = A->n;
    NLOLDuint j, ii;
    __NLOLDRowColumn* Cj = NULL;
...
static void __nlOLDMatrixVectorProd_default(NLOLDfloat* x, NLOLDfloat* y) { __nlOLDSparseMatrixMult(&(__nlOLDCurrentContext->M), x, y); }
...
void nlOLDMakeCurrent(NLOLDContext context) { __nlOLDCurrentContext = (__NLOLDContext*)(context); }

NLOLDContext nlOLDGetCurrent(void) { return __nlOLDCurrentContext; }

static void __nlOLDCheckState(NLOLDenum state) { __nlOLD_assert(__nlOLDCurrentContext->state == state); }
...
static void __nlOLDBeginSystem()
{
    ...
    __nlOLDCurrentContext->variable = __NLOLD_NEW_ARRAY(__NLOLDVariable, __nlOLDCurrentContext->nb_variables);
    __nlOLDCurrentContext->alloc_variable = NLOLD_TRUE;
}

static void __nlOLDEndSystem() { __nlOLDTransition(__NLOLD_STATE_MATRIX_CONSTRUCTED, __NLOLD_STATE_SYSTEM_CONSTRUCTED); }
...
static void __nlOLDEndRow()
{
    ...
    if(__nlOLDCurrentContext->least_squares)
    {
        for(i = 0; i < nf; i++)
        {
            for(j = 0; j < nf; j++)
            {
                __nlOLDSparseMatrixAdd(M, af->coeff[i].index, af->coeff[j].index, af->coeff[i].value * af->coeff[j].value);
            }
        }
    ...
    else
    {
        for(i = 0; i < nf; i++)
        {
            __nlOLDSparseMatrixAdd(M, current_row, af->coeff[i].index, af->coeff[i].value);
        }
...
static NLOLDboolean __nlOLDSolve_SUPERLU(NLOLDboolean do_perm)
{
     ...
     */
    sCreate_CompCol_Matrix(&A, n, n, nnz, a, asub, xa, SLU_NR, /* Row_wise, no supernode */
                           SLU_S,                              /* floats                */
                           SLU_GE                              /* general storage        */
    );

    /* Step 2: create vector */
    sCreate_Dense_Matrix(&B, n, 1, b, n, SLU_DN, /* Fortran-type column-wise storage */
                         SLU_S,                  /* floats                          */
                         SLU_GE                  /* general                          */
    );
