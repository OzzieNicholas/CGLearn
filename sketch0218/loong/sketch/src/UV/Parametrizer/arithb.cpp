...
float Inpf(float* v1, float* v2) { return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]; }
...
void Mat3Inv(float m1[][3], float m2[][3])
{
    ...
    det = m2[0][0] * (m2[1][1] * m2[2][2] - m2[1][2] * m2[2][1]) - m2[1][0] * (m2[0][1] * m2[2][2] - m2[0][2] * m2[2][1]) +
...
void Mat4CpyMat4(float m1[][4], float m2[][4]) { memcpy(m1, m2, 4 * 4 * sizeof(float)); }
...
void Mat3MulSerie(float answ[][3], float m1[][3], float m2[][3], float m3[][3], float m4[][3], float m5[][3], float m6[][3], float m7[][3],
                  float m8[][3])
{
    float temp[3][3];
...
void Mat4MulSerie(float answ[][4], float m1[][4], float m2[][4], float m3[][4], float m4[][4], float m5[][4], float m6[][4], float m7[][4],
                  float m8[][4])
{
    float temp[4][4];
...
void Mat4Clr(float* m) { memset(m, 0, 4 * 4 * sizeof(float)); }

void Mat3Clr(float* m) { memset(m, 0, 3 * 3 * sizeof(float)); }
...
void Mat4MulFloat(float* m, float f)
{
    ...
    for(i = 0; i < 12; i++)
        m[i] *= f; /* count to 12: without vector component */
}
...
void printvecf(char* str, float v[3]) { printf("%s: %.3f %.3f %.3f\n", str, v[0], v[1], v[2]); }

void printquat(char* str, float q[4]) { printf("%s: %.3f %.3f %.3f %.3f\n", str, q[0], q[1], q[2], q[3]); }
...
float QuatDot(float* q1, float* q2) { return q1[0] * q2[0] + q1[1] * q2[1] + q1[2] * q2[2] + q1[3] * q2[3]; }
...
float* vectoquat(float* vec, short axis, short upflag)
{
    ...
    if(axis != upflag)
    {
        ...
        {
            if(upflag == 1)
                hoek = (float)(0.5 * atan2(fp[2], fp[1]));
            else
                hoek = (float)(-0.5 * atan2(fp[1], fp[2]));
        }
        else if(axis == 1)
        {
            if(upflag == 0)
                hoek = (float)(-0.5 * atan2(fp[2], fp[0]));
            else
                hoek = (float)(0.5 * atan2(fp[0], fp[2]));
        }
        else
        {
            if(upflag == 0)
                hoek = (float)(0.5 * atan2(-fp[1], -fp[0]));
            else
                hoek = (float)(-0.5 * atan2(-fp[0], -fp[1]));
        }
...
void i_window(float left, float right, float bottom, float top, float nearClip, float farClip, float mat[][4])
{
    float Xdelta, Ydelta, Zdelta;
...
float VecLength(float* v) { return (float)sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]); }
...
float Inp2f(float* v1, float* v2) { return v1[0] * v2[0] + v1[1] * v2[1]; }
...
void spheremap(float x, float y, float z, float* u, float* v)
{
    ...
    if(len > 0.0)
    {
        ...
        if(x == 0.0 && y == 0.0)
            ...
            *u = (float)((1.0 - atan2(x, y) / M_PI) / 2.0);
...
/* *****************  m1 = m2 *****************  */
void cpy_m3_m3(float m1[][3], float m2[][3]) { memcpy(m1[0], m2[0], 9 * sizeof(float)); }

/* *****************  m1 = m2 *****************  */
void cpy_m4_m4(float m1[][4], float m2[][4]) { memcpy(m1[0], m2[0], 16 * sizeof(float)); }
