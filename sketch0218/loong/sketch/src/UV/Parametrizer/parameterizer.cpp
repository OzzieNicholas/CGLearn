#include "sketch/UV/Parametrizer/MEM_guardedalloc.h"

#include "sketch/UV/Parametrizer/BLI_arithb.h"
#include "sketch/UV/Parametrizer/BLI_memarena.h"
// #include "BLI_rand.h"
#include "sketch/UV/Parametrizer/BKE_utildefines.h"
#include "sketch/UV/Parametrizer/BLI_boxpack2d.h"
#include "sketch/UV/Parametrizer/BLI_heap.h"
...
#include "sketch/UV/Parametrizer/ONL_opennl.h"
...
static int PHashSizes[] = { 1,       3,       5,       11,      17,       37,       67,       131,       257,      521,
...
static int phash_size(PHash* ph) { return ph->size; }
...
static float p_face_uv_area_signed(PFace* f)
{
    ...
    return 0.5f * (((v2->uv[0] - v1->uv[0]) * (v3->uv[1] - v1->uv[1])) - ((v3->uv[0] - v1->uv[0]) * (v2->uv[1] - v1->uv[1])));
}

static float p_face_uv_area(PFace* f) { return fabs(p_face_uv_area_signed(f)); }
...
static PEdge* p_wheel_edge_next(PEdge* e) { return e->next->next->pair; }

static PEdge* p_wheel_edge_prev(PEdge* e) { return (e->pair) ? e->pair->next : NULL; }

static PEdge* p_boundary_edge_next(PEdge* e) { return e->next->vert->edge; }
...
static bool p_vert_interior(PVert* v) { return ((bool)(v->edge->pair != NULL)); }
    ...
    *pair = NULL;
    ...
    while(pe)
    {
        if(pe != e)
        {
            ...
            PHashKey v1_key = v1->u.key;
            PHashKey v2_key = v2->u.key;

            if(((v1_key == key2) && (v2_key == key1)) || ((v1_key == key1) && (v2_key == key2)))
            {
    ...
    if((handle->chart_index) && (*pair))
    {
        int f_index = e->face->idx;
        int pair_f_index = (*pair)->face->idx;

        // std::cout<<f_index<<" "<<pair_f_index<<std::endl;

        if((*handle->chart_index)[f_index] != (*handle->chart_index)[pair_f_index])
        {
            *pair = NULL;
            return P_FALSE;
        }
...
static PFace* p_face_add_construct(PHandle* handle, ParamKey key, ParamKey* vkeys, float* co[3], float* uv[3], int i1, int i2, int i3, bool* pin,
                                   bool* select)// ParamBool
{
...
void param_pack(ParamHandle* handle, float margin)
{
    ...
    boxarray = (boxPack*)MEM_mallocN(phandle->ncharts * sizeof(boxPack), "boxPack box");
...
void param_delete(ParamHandle* handle)
{
    ...
    param_assert((phandle->state == PHANDLE_STATE_ALLOCATED) || (phandle->state == PHANDLE_STATE_CONSTRUCTED));
...
void param_face_add(ParamHandle* handle, ParamKey key, int nverts, ParamKey* vkeys, float** co, float** uv, bool* pin, bool* select)// ParamBool
{
    PHandle* phandle = (PHandle*)handle;
...
void param_construct_data_end(ParamHandle* handle, bool fill, bool impl)// ParamBool
{
...
void param_pack(ParamHandle* handle) { p_charts_pack((PHandle*)handle); }
