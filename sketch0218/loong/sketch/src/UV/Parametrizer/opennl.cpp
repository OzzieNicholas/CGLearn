...
{
    fprintf(stderr, "OpenNL assertion failed: %s, file:%s, line:%d\n", cond, file, line);
    abort();
}

static void __nl_range_assertion_failed(float x, float min_val, float max_val, char* file, int line)
{
    fprintf(stderr, "OpenNL range assertion failed: %f in [ %f ... %f ], file:%s, line:%d\n", x, min_val, max_val, file, line);
    abort();
}

static void __nl_should_not_have_reached(char* file, int line)
{
    fprintf(stderr, "OpenNL should not have reached this point: file:%s, line:%d\n", file, line);
    abort();
}
...
#define __nl_range_assert(x, min_val, max_val)                                                                                                       \
    {                                                                                                                                                \
        ...
        {                                                                                                                                            \
            __nl_range_assertion_failed(x, min_val, max_val, __FILE__, __LINE__);                                                                    \
        }                                                                                                                                            \
    }
...
#define __NL_DELETE(x)                                                                                                                               \
    free(x);                                                                                                                                         \
    x = NULL
#define __NL_DELETE_ARRAY(x)                                                                                                                         \
    free(x);                                                                                                                                         \
    x = NULL
...
static void __nlRowColumnZero(__NLRowColumn* c) { c->size = 0; }
...
static void __nlMatrixVectorProd_default(NLfloat* x, NLfloat* y) { __nlSparseMatrixMult(&(__nlCurrentContext->M), x, y); }
...
void nlMakeCurrent(NLContext context) { __nlCurrentContext = (__NLContext*)(context); }

NLContext nlGetCurrent(void) { return __nlCurrentContext; }
...
static void __nlBeginSystem()
{
    ...
    else
    {
        ...
        __nlCurrentContext->variable = __NL_NEW_ARRAY(__NLVariable, __nlCurrentContext->nb_variables);
...
static void __nlEndRow()
{
    ...
    if(__nlCurrentContext->least_squares)
    {
        if(!__nlCurrentContext->solve_again)
        {
            for(i = 0; i < nf; i++)
            {
                for(j = 0; j < nf; j++)
                {
                    __nlSparseMatrixAdd(M, af->coeff[i].index, af->coeff[j].index, af->coeff[i].value * af->coeff[j].value);
                }
            }
        }
    ...
    else
    {
        if(!__nlCurrentContext->solve_again)
        {
            for(i = 0; i < nf; i++)
            {
                __nlSparseMatrixAdd(M, current_row, af->coeff[i].index, af->coeff[i].value);
            }
        }
...
static NLboolean __nlFactorize_SUPERLU(__NLContext* context, NLint* permutation)
{
    ...
    sCreate_CompCol_Matrix(&At, n, n, nnz, a, asub, xa, SLU_NC, /* Colum wise, no supernode */
                           SLU_S,                               /* floats */
    ...
    sgstrf(&options, &AtP, relax, panel_size, etree, NULL, 0, context->slu.perm_c, context->slu.perm_r, &(context->slu.L), &(context->slu.U),
...
static NLboolean __nlInvert_SUPERLU(__NLContext* context)
{
    ...
    sgstrs(TRANS, &(context->slu.L), &(context->slu.U), context->slu.perm_c, context->slu.perm_r, &B, &(context->slu.stat), &info);
