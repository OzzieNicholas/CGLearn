...
void StatInit(SuperLUStat_t* stat)
{
    ...
    if(!stat->utime)
        ABORT("SUPERLU_MALLOC fails for stat->utime");
...
float SpaSize(int n, int np, float sum_npw) { return (sum_npw * 8 + np * 8 + n * 4) / 1024.0f; }

float DenseSize(int n, float sum_nw) { return (sum_nw * 8 + n * 8) / 1024.0f; }
