...
void sCreate_CompCol_Matrix(SuperMatrix* A, int m, int n, int nnz, float* nzval, int* rowind, int* colptr, Stype_t stype, Dtype_t dtype,
                            Mtype_t mtype)
{
    NCformat* Astore;
...
void sCreate_CompRow_Matrix(SuperMatrix* A, int m, int n, int nnz, float* nzval, int* colind, int* rowptr, Stype_t stype, Dtype_t dtype,
                            Mtype_t mtype)
{
    NRformat* Astore;
...
void sCreate_SuperNode_Matrix(SuperMatrix* L, int m, int n, int nnz, float* nzval, int* nzval_colptr, int* rowind, int* rowind_colptr,
                              int* col_to_sup, int* sup_to_col, Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    SCformat* Lstore;
...
void sPrint_SuperNode_Matrix(char* what, SuperMatrix* A)
{
    ...
    printf("\nnzval_colptr: ");
    for(i = 0; i <= n; ++i)
        printf("%d  ", Astore->nzval_colptr[i]);
    printf("\nrowind: ");
    for(i = 0; i < Astore->rowind_colptr[n]; ++i)
        printf("%d  ", Astore->rowind[i]);
    printf("\nrowind_colptr: ");
    for(i = 0; i <= n; ++i)
        printf("%d  ", Astore->rowind_colptr[i]);
    printf("\ncol_to_sup: ");
    for(i = 0; i < n; ++i)
        printf("%d  ", col_to_sup[i]);
    printf("\nsup_to_col: ");
...
void sFillRHS(trans_t trans, int nrhs, float* x, int ldx, SuperMatrix* A, SuperMatrix* B)
{
    ...
    sp_sgemm(transc, nrhs, one, A, x, ldx, zero, rhs, ldc);
}
...
void sPrintPerf(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage, float rpg, float rcond, float* ferr, float* berr, char* equed,
                SuperLUStat_t* stat)
{
    SCformat* Lstore;
    NCformat* Ustore;
    ...
    printf("PERF:%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f\n", utime[FACT], ops[FACT] * 1e-6 / utime[FACT], utime[SOLVE],
           ops[SOLVE] * 1e-6 / utime[SOLVE], utime[ETREE], utime[EQUIL], utime[RCOND], utime[REFINE]);
