...
int sQuerySpace(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage)
{
    ...
    /* For LU factors */
    mem_usage->for_lu = (float)((4 * n + 3) * iword + Lstore->nzval_colptr[n] * dword + Lstore->rowind_colptr[n] * iword);
    mem_usage->for_lu += (float)((n + 1) * iword + Ustore->colptr[n] * (dword + iword));

    /* Working storage to support factorization */
    mem_usage->total_needed = mem_usage->for_lu + (float)((2 * panel_size + 4 + NO_MARKER) * n * iword + (panel_size + 1) * n * dword);
               ...
               int** iwork, float** dwork)
{
    ...
    if(!expanders)
        ABORT("SUPERLU_MALLOC fails for expanders");
...
int sLUWorkInit(int m, int n, int panel_size, int** iworkptr, float** dworkptr, LU_space_t MemModel)
{
    ...
    float* old_ptr;
    int maxsuper = sp_ienv(3), rowblk = sp_ienv(4);
 ...
 */
int sLUMemXpand(int jcol, int next, /* number of elements currently in the factors */
 ...
 */
void* sexpand(int* prev_len,   /* length used from previous call */
    ...
    { /* MemModel == USER */
        ...
        else
        {
            ...
            if(type != USUB)
            {
                ...
                bytes_to_copy = (char*)stack.array + stack.top1 - (char*)expanders[type + 1].mem;
...
int smemory_usage(const int nzlmax, const int nzumax, const int nzlumax, const int n)
{
    ...
    return (10 * n * iword + nzlmax * iword + nzumax * (iword + dword) + nzlumax * dword);
}
