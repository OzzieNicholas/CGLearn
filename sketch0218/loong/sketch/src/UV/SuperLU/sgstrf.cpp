...
void sgstrf(superlu_options_t* options, SuperMatrix* A, int relax, int panel_size, int* etree, void* work, int lwork, int* perm_c, int* perm_r,
            SuperMatrix* L, SuperMatrix* U, SuperLUStat_t* stat, int* info)
{
    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, L, U, &Glu, &iwork, &swork);
    if(*info)
        return;
    ...
    for(jcol = 0; jcol < min_mn;)
    {
        ...
        { /* Work on one panel of panel_size columns */
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++)
            {
                ...
                if((*info = scolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k], segrep, &repfnz[k], xprune, marker, parent, xplore, &Glu)) != 0)
                    return;

                /* Numeric updates */
                if((*info = scolumn_bmod(jj, (nseg - nseg1), &dense[k], tempv, &segrep[nseg1], &repfnz[k], jcol, &Glu, stat)) != 0)
                    return;
    ...
    *info = iinfo;
    ...
    {
        sCreate_SuperNode_Matrix(L, A->nrow, A->ncol, nnzL, Glu.lusup, Glu.xlusup, Glu.lsub, Glu.xlsub, Glu.supno, Glu.xsup, SLU_SC, SLU_S, SLU_TRLU);
        sCreate_CompCol_Matrix(U, min_mn, min_mn, nnzU, Glu.ucol, Glu.usub, Glu.xusub, SLU_NC, SLU_S, SLU_TRU);
    }
