...
static void get_colamd(const int m,   /* number of rows in matrix A. */
...
static void getata(const int m,      /* number of rows in matrix A. */
                   const int n,      /* number of columns in matrix A. */
...
static void at_plus_a(const int n,    /* number of columns in matrix A. */
                      const int nz,   /* number of nonzeros in matrix A */
                      int* colptr,    /* column pointer of size n+1 for matrix A. */
                      int* rowind,    /* row indices of size nz for matrix A. */
 ...
 */
{
    ...
    int m, n, bnz, *b_colptr = nullptr, i;
    int delta, maxint, nofsub, *invp;
    int *b_rowind = nullptr, *dhead, *qsize, *llist, *marker;
...
#endif
        return;
    case 1: /* Minimum degree ordering on A'*A */ getata(m, n, Astore->nnz, Astore->colptr, Astore->rowind, &bnz, &b_colptr, &b_rowind);
#if(PRNTlevel >= 1)
    ...
    case 2: /* Minimum degree ordering on A'+A */
        ...
        /*printf("Form A'+A time = %8.3f\n", t);*/
        break;
    case 3: /* Approximate minimum degree column ordering. */ get_colamd(m, n, Astore->nnz, Astore->colptr, Astore->rowind, perm_c);
#if(PRNTlevel >= 1)
