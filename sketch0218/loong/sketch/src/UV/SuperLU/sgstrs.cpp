 ...
 */
    ...
    *info = 0;
    ...
    if(trans != NOTRANS && trans != TRANS && trans != CONJ)
        *info = -1;
    else if(L->nrow != L->ncol || L->nrow < 0 || L->Stype != SLU_SC || L->Dtype != SLU_S || L->Mtype != SLU_TRLU)
        *info = -2;
    else if(U->nrow != U->ncol || U->nrow < 0 || U->Stype != SLU_NC || U->Dtype != SLU_S || U->Mtype != SLU_TRU)
        *info = -3;
    else if(ldb < SUPERLU_MAX(0, L->nrow) || B->Stype != SLU_DN || B->Dtype != SLU_S || B->Mtype != SLU_GE)
        *info = -6;
    if(*info)
    {
        i = -(*info);
    ...
    if(!work)
        ABORT("Malloc fails for local work[].");
    soln = floatMalloc(n);
    if(!soln)
        ABORT("Malloc fails for local soln[].");
 ...
 */
void sprint_soln(int n, float* soln)
{
    int i;
